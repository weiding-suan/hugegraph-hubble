...
import com.baidu.hugegraph.loader.executor.LoadManager;
...
public class LoadTaskService {
    ...
    public GraphStruct buildGraphStruct(FileMapping fileMapping) {
        ...
        AdvancedSetting advancedSetting = fileMapping.getAdvancedSetting();
        source.charset(advancedSetting.getCharset());
        // NOTE: format and delimiter must be CSV and "," temporarily
        source.format(FileFormat.valueOf(advancedSetting.getFormat()));
        source.delimiter(advancedSetting.getDelimiter());
        source.dateFormat(advancedSetting.getDateFormat());
        source.timeZone(advancedSetting.getTimeZone());
        source.skippedLine(new SkippedLine(advancedSetting.getSkippedLine()));
    ...
    public void handleLoadTask(LoadManager manager, LoadAction action) {
        switch (action) {
            case START:
                manager.startLoad();
                break;
